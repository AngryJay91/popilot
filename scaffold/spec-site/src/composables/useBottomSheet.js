...
export function useBottomSheet() {
    ...
    function onPointerUp(e) {
        ...
        if (Math.abs(velocity) > VELOCITY_THRESHOLD) {
            ...
            const fullT = 0;
            const distances = [
                { state: 'peek', d: Math.abs(currentTranslate - peekT) },
                { state: 'half', d: Math.abs(currentTranslate - halfT) },
                { state: 'full', d: Math.abs(currentTranslate - fullT) },
            ];
