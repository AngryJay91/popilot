...
import { FALLBACK_SPRINTS, FALLBACK_EPICS, } from './navTypes';
// ── Singleton state ──
export const sprints = ref([...FALLBACK_SPRINTS]);
export const epics = ref([...FALLBACK_EPICS]);
...
export function getPagesByCategory(sprint, category) {
...
export async function loadNavData() {
    ...
    const { data, error } = await apiGet('/api/v2/nav');
    if (error || !data)
        return;
...
export async function updateSprint(id, data) {
    if (isStaticMode())
        return { error: 'CRUD not available in static mode' };
    const { error } = await apiPatch(`/api/v2/nav/sprints/${id}`, data);
    if (error)
        return { error };
    await loadNavData();
    return {};
...
export async function setActiveSprint(id) {
    if (isStaticMode())
        return { error: 'CRUD not available in static mode' };
...
// ── CRUD: Epics ──
export async function addEpic(sprint, data) {
    if (isStaticMode())
        return { error: 'CRUD not available in static mode' };
...
export async function updateEpic(sprint, epicId, data) {
    if (isStaticMode())
        return { error: 'CRUD not available in static mode' };
    const { error } = await apiPatch(`/api/v2/nav/epics/${sprint}/${epicId}`, data);
    if (error)
        return { error };
    await loadNavData();
    return {};
}
export async function deleteEpic(sprint, epicId) {
    if (isStaticMode())
        return { error: 'CRUD not available in static mode' };
...
export async function carryOverEpic(uid, targetSprint, newEpicId, newLabel, newBadge) {
    if (isStaticMode())
        return { error: 'CRUD not available in static mode' };
    const epic = epics.value.find(e => e.uid === uid);
    if (!epic)
        return { error: `Epic not found: ${uid}` };
