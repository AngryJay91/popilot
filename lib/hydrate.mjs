...
export async function hydrate(targetDir, opts = {}) {
  ...
  const ctx = buildContext(projectYaml, capabilities, userYaml);
  ...
  // System .hbs files
  const systemFiles = [
    { path: join(targetDir, 'CLAUDE.md.hbs'), name: 'CLAUDE.md' },
    { path: join(targetDir, '.context', 'WORKFLOW.md.hbs'), name: 'WORKFLOW.md' },
  ];
  ...
  const domainResults = [];
  const domainTemplatePath = join(commandsDir, '_domain.md.hbs');
  let domainTemplate;
  try {
 ...
 */
function buildContext(projectYaml, capabilities, userYaml = {}) {
